...
| D24 | **The query vocabulary advertised fields the executor could not answer.** `ssn` and `dob` were in `contracts.query_plan_schema`'s enum with no branch in `app._apply_filter`, so the planner emitted a valid plan, the haystack stayed empty, nothing matched, and the user was told *“no such entity”* for an identifier the store held — a capability gap reading as a factual answer. `vin` was not offered at all, and `ssn`/`vin`/`dob` never reached the dossier identity block, so they were invisible to the app, the agent and the identifier index | me, via T0.7 | ✅ **fixed** — wired end to end; `UnservedQueryField` raises on drift and the smoke test asserts the two vocabularies are the same set |
...
| D33 | **`identifier_kind=dob` absorbs any bare date, not just birthdates, and a non-personal date can be bound to a person as if it were their own DOB.** In `DOC00000`, a deposition date (`2024-03-28`) is bound to the claimant `Robert Miller` via the line-proximity fallback -- his name is simply the nearest one on the page -- even though the deposition has nothing to do with him. Root cause: `IDENTIFIER_LABEL_TO_PREDICATE` maps both `date` and `date_written` to `has_dob`, so a payment date, a coverage-denial date, and a deposition date all become `kind=dob` observations alongside a real birthdate. Found by the user during manual golden-set validation, cross-checking `DOC00000` by hand against `validation/system_output/DOC00000.txt` | user, via manual validation | open |
| D34 | **46% of labeled mentions sit inside an entity that fuses MULTIPLE REAL ENTITIES, and B-cubed did not reveal it.** Measured on the 60-doc slice: 9 of 40 predicted entities each contain 2–4 distinct ground-truth entities, and 24 of 40 carry conflicting `entity_class` labels. One entity fuses `Marcus Lopez` (attorney) + `Peter Lopez` (claimant) + `Thomas Collision Center` + `Lopez Collision Center`; another fuses four different Andersons. Driver is the **person-name comparison model applied to organizations** — `first_name`/`last_name` are `tokens[0]`/`tokens[-1]`, so `Thomas Collision Center` and `Lopez Collision Center` both block on `last_name="center"` and score 0.80 on a shared `"center collision "` prefix under Jaro-Winkler over sorted tokens — exactly the defect ARCHITECTURE.md already documents and names the entity_type/role split (T1.1) as the fix for. Transitive connected components then chains persons in via shared surname (D13). **Removing today's containment level and restoring the person_vs_org veto barely helps: 45.9% → 40.4%.** This is pre-existing and structural, not caused by today's work, though today's changes made it modestly worse | user, via manual validation of DOC00047 | open — **blocks any headline resolution claim** |
...
| ⚠ **entities vs ground truth @ 0.45** | **42–49 vs 42 — the ratio is MEANINGLESS** | **Retracted as a quality claim (D34).** Over-merges and fragmentation cancel out: 9 of 40 entities fuse multiple real entities while others fragment, netting to roughly the right COUNT with the wrong composition. The count also moves 42→49 between runs. Use over-merge rate, not entity count |
